...
export async function signInAction(previous: SignInState, formData: FormData): Promise<SignInState> {
  ...
  const { email, password } = parsed.data; let token: string;
  try { token = (await signIn({ email, password })).token; } catch (error) { if (error instanceof SigninApiError) return { status: "error", email, message: error.code === "invalid-credentials" ? "Email or password is incorrect." : "We could not sign you in. Please try again." }; throw error; }
  try { await setSession(token); } catch (error) { if (error instanceof SessionValidationError || error instanceof EnvironmentValidationError) return { status: "error", email, message: "We couldn't start a secure session. Please try again." }; throw error; }
