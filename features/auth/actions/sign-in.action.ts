...
export async function signInAction(previous: SignInState, formData: FormData): Promise<SignInState> {
  const parsed = parseSignInFormData(formData);
  const returnTo = normalizeReturnTo(formData.get("returnTo"));
  if (!parsed.success) return { status: "error", email: parsed.email, message: parsed.message };
  const { email, password } = parsed.data;
  let signInResult: { token: string; user?: { name: string; email: string } };
  try {
    signInResult = await signIn({ email, password });
  } catch (error) {
    if (error instanceof SigninApiError) {
      return {
        status: "error",
        email,
        message: error.code === "invalid-credentials" ? "Email or password is incorrect." : "We could not sign you in. Please try again.",
      };
    }
    throw error;
  }
  try {
    const user = signInResult.user?.name && signInResult.user?.email
      ? { name: signInResult.user.name, email: signInResult.user.email }
      : { name: email.split("@")[0] || "User", email };
    await setSession(signInResult.token, user);
  } catch (error) {
    if (error instanceof SessionValidationError || error instanceof EnvironmentValidationError) {
      return { status: "error", email, message: "We couldn't start a secure session. Please try again." };
    }
    throw error;
  }
  redirect(returnTo);
