...
|---|---|---|---|---|---|
| `API-001` / PRD A1 | What is the API base URL? | **Confirmed** | An anonymous `GET https://ecommerce.routemisr.com/api/v1/categories` returned HTTP `200` on 2026-07-31. This confirms that the planning base served the observed endpoint at that time; it does not guarantee future availability. Evidence: `docs/api/examples/categories/get-all-categories.md`. | D10 records the environment value; revalidate before T06/T07. | All live integration if the host later fails or redirects unexpectedly. |
| `API-002` / PRD A2 | Are category, subcategory, brand, and product GETs public? | **Provisional** | `GET /categories` returned HTTP `200` without authentication on 2026-07-31. The other eight catalog GET candidates remain unverified, so the broader public-access decision stays Provisional. Evidence: `docs/api/examples/categories/get-all-categories.md`. | F03, F04. | C01–C09 for any endpoint that does not allow anonymous access. |
...
| `API-004` / PRD A9 | What are the success/error envelopes, statuses, and field types? | **Open** | A 2026-07-31 HTTP `200` categories response had top-level `results` (number), `metadata` (object), and `data` (array); its representative item exposed `_id`, `name`, `slug`, `image`, `createdAt`, and `updatedAt` as strings. Other success/error contracts, optionality, and stability remain unverified. Evidence: `docs/api/examples/categories/get-all-categories.md`. | F03–F13, then T00. | All response adapters and UI data models. |
