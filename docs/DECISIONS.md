...
|---|---|---|---|---|---|
| `API-001` / PRD A1 | What is the API base URL? | **Confirmed** | On 2026-07-31, all seven F03 taxonomy/brand GETs returned HTTP `200` JSON responses from `https://ecommerce.routemisr.com/api/v1` without redirects. This confirms the base for those observations, not future availability. Evidence: `docs/api/examples/F03_SUMMARY.md`. | D10 records the environment value; revalidate before T06/T07. | All live integration if the host later fails or redirects unexpectedly. |
| `API-002` / PRD A2 | Are category, subcategory, brand, and product GETs public? | **Provisional** | All seven category, subcategory, and brand GETs returned HTTP `200` without authentication on 2026-07-31. The two product GET candidates remain unverified, so the broader catalog-public decision stays Provisional. Evidence: `docs/api/examples/F03_SUMMARY.md`. | F04. | C01–C09 for any endpoint that does not allow anonymous access. |
| `API-003` / PRD A5 | What do all path IDs represent? | **Open** | In F03, the first category ID from `GET /categories` successfully addressed category detail and category-scoped subcategories; the first subcategory and brand IDs from their list responses successfully addressed their detail endpoints, and each detail `data._id` matched. Other resource-ID semantics remain unverified. Evidence: `docs/api/examples/F03_SUMMARY.md`. | F04, F09–F12. | Detail routes and mutations using `{id}` outside the verified taxonomy relationships. |
| `API-004` / PRD A9 | What are the success/error envelopes, statuses, and field types? | **Open** | F03 observed HTTP `200` list envelopes with numeric `results`, object `metadata`, and array `data`; detail envelopes contained an object `data`. Category/brand resources exposed string `_id`, `name`, `slug`, `image`, `createdAt`, and `updatedAt`; subcategories exposed string `_id`, `name`, `slug`, `category`, `createdAt`, and `updatedAt`; detail resources also exposed numeric `__v`. Errors, optionality, stability, and other domains remain unverified. Evidence: `docs/api/examples/F03_SUMMARY.md` and its seven records. | F04–F13, then T00. | Unobserved response adapters and UI data models. |
| `API-005` | What pagination envelopes, defaults, and bounds are supported? | **Open** | No-query F03 list responses exposed numeric `metadata.currentPage`, `metadata.numberOfPages`, and `metadata.limit`; subcategory and brand observations also exposed numeric `metadata.nextPage`. No pagination input or later-page behavior was tested, so defaults, bounds, and semantics remain Open. Evidence: the F03 list records. | F04–F05. | Pagination controls and list adapters. |
...
| `API-007` | Should GET/DELETE bodies and trailing slashes be normalized? | **Provisional** | All seven F03 GETs succeeded with normalized paths, no body, and no trailing slash. This supports that transport shape for the observed taxonomy/brand endpoints; DELETE behavior and tolerance elsewhere remain unverified. Preserve exact endpoint segment spelling such as `forgotPasswords`. Evidence: `docs/api/examples/F03_SUMMARY.md`. | F04–F12. | Request transport serialization outside the observed endpoints. |
