...
| `API-004` / PRD A9 | What are the success/error envelopes, statuses, and field types? | **Open** | F03 established taxonomy list/detail shapes. F04 observed a product list envelope with numeric `results`, object `metadata`, and array `data`, plus a detail envelope with object `data`. Common product fields included string IDs/text/timestamps, numeric `sold`, `ratingsQuantity`, `quantity`, `price`, and `ratingsAverage`, string-array `images`, subcategory array, category/brand objects, and HTTPS image strings; detail also exposed numeric `__v` and a `reviews` array with redacted nested user structure. Errors, optionality, stability, semantics, and protected domains remain unverified. Evidence: `docs/api/examples/products/F04_SUMMARY.md` and its two records. | F05–F13, then T00. | Unobserved response adapters and UI data models. |
| `API-005` | What pagination envelopes, defaults, and bounds are supported? | **Open** | No-query F03 and product-list observations exposed numeric `metadata.currentPage`, `metadata.numberOfPages`, and `metadata.limit`; multi-page observations also exposed numeric `metadata.nextPage`. No pagination input or later-page behavior was tested, so defaults, bounds, and semantics remain Open. Evidence: the F03/F04 list records. | F05. | Pagination controls and list adapters. |
