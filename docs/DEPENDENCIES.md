...
Zod and `server-only` are the newly approved production dependencies. D08A approved `server-only@0.0.1` documentation-only; D09A installed and verified it as an exact runtime dependency before D10 begins.
