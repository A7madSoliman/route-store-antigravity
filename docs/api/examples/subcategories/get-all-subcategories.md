...
|---|---|
| Request name | `Get All SubCategories` |
| HTTP method | `GET` |
| Request URL | `https://ecommerce.routemisr.com/api/v1/subcategories` |
| Normalized endpoint | `/subcategories` |
...
| Response time | `89.78 ms` |
| Safe response headers | `Content-Type: application/json; charset=utf-8` |
| Sanitization note | The response was parsed in process memory only. The full list was not emitted or written; the example retains one item and replaces every identifier, text value, and timestamp. |
...
| `$.data[0].category` | string | Value replaced with `<category-id>` |
...
- Relationships beyond the observed string-valued `category` field remain unverified.

## Related decisions

- `API-001` — the confirmed base URL served this endpoint.
- `API-002` — this list request succeeded anonymously.
- `API-003` — the first returned subcategory ID was used for its detail request.
