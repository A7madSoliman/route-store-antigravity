...
describe("getOrders endpoint adapter", () => {
  ...
  it("returns mapped orders list on successful response using fixture", async () => {
    vi.mocked(sessionServer.getSessionToken).mockResolvedValue(validJwt);
    vi.mocked(protectedRequest.protectedGet).mockResolvedValue(getOrdersFixture);
    ...
    expect(orders[0].numericId).toBe(getOrdersFixture[0].id);
    expect(orders[0].totalOrderPrice).toBe(getOrdersFixture[0].totalOrderPrice);
    expect(orders[0].cartItems[0].product?.title).toBe(getOrdersFixture[0].cartItems[0].product.title);
    expect(orders[0].shippingAddress.city).toBe(getOrdersFixture[0].shippingAddress.city);
  });
