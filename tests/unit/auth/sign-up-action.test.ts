...
vi.mock("server-only", () => ({}));
const { normalizeReturnToMock } = vi.hoisted(() => ({
  normalizeReturnToMock: vi.fn((value: unknown) => (typeof value === "string" && value.startsWith("/") && !value.startsWith("//") ? value : "/")),
}));
...
describe("signup action", () => {
  it("returns a local mismatch without calling the endpoint", async () => {
    ...
    data.set("name", "Name");
    data.set("email", "email@example.test");
    data.set("phone", "1");
    data.set("password", "one");
    data.set("rePassword", "two");
  ...
  it("rejects whitespace-only input and preserves surrounding whitespace", () => {
    ...
    preserved.set("name", " John ");
    preserved.set("email", " email@example.test ");
    preserved.set("phone", " 1 ");
    preserved.set("password", " p ");
    preserved.set("rePassword", " p ");
    expect(parseSignUpFormData(preserved)).toMatchObject({
      success: true,
      data: { name: " John ", email: " email@example.test ", phone: " 1 " },
    });
  });
  ...
  it("returns account-created after expected session validation failure", async () => {
    vi.mocked(signUp).mockResolvedValueOnce({
      token: "synthetic",
      user: { name: "Name", email: "email@example.test" },
    });
    vi.mocked(setSession).mockRejectedValueOnce(
      new (await import("@/lib/auth/session-codec.server")).SessionValidationError(),
    );
    const data = new FormData();
    data.set("name", "Name");
    data.set("email", "email@example.test");
    data.set("phone", "1");
    data.set("password", "same");
    data.set("rePassword", "same");
    await expect(
      signUpAction({ status: "idle", name: "", email: "", phone: "" }, data),
  ...
  it("calls setSession with token and user identity on successful signup", async () => {
    vi.mocked(signUp).mockResolvedValueOnce({
      token: "token123",
      user: { name: "New User", email: "new@example.test" },
    });
    const data = new FormData();
    data.set("name", "New User");
    data.set("email", "new@example.test");
    data.set("phone", "01012345678");
    data.set("password", "samePass123");
    data.set("rePassword", "samePass123");
    await signUpAction({ status: "idle", name: "", email: "", phone: "" }, data);
